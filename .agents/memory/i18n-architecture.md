...
**Two gotchas beyond wrapping strings** (caught in review on the dashboard page): (1) `toLocaleDateString("fr-FR", …)` and similar hardcoded locales must become language-aware (`language === "en" ? "en-US" : "fr-FR"`) — get `language` from `useI18n()`, not `useT()`. (2) When `t()` is called inside a `useCallback`/`useMemo` (e.g. toast messages), add `t` to the dependency array or the message stays in the old language after a runtime switch. The chat/dashboard surface is `dashboard.tsx`, a separate file from `home.tsx` — don't assume the in-app home is the landing.
